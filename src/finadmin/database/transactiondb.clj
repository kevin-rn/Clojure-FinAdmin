...
(defn get-expense-transactions-by-email
  "Retrieves expense data along with the associated transaction details for a specific account based on the account's email.
    ...
    Parameters:
    - account-email: The email address of the account to retrieve the expenses for.
  
    Returns:
    - A sequence of maps, where each map represents an expense and its associated transaction details, without the ID columns."
  [email]
  (jdbc/execute! database-connection
                 ["SELECT *
                    FROM transactions t 
                    INNER JOIN expenses e ON t.transaction_id = e.transaction_id 
...
(defn get-invoice-transactions-by-email
  ...
  (jdbc/execute! database-connection
                 ["SELECT *
                      FROM transactions t 
                      INNER JOIN invoices i ON t.transaction_id = i.transaction_id 
                      WHERE t.account_email = ?
                      ORDER BY t.transaction_date DESC"
                  email]))
